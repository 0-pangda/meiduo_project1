...
class DetailVisitView(View):
    ...
    def post(self, request, category_id):
        ...
        today_date = datetime.strptime(today_str, '%Y-%m-%d')  # 时间字符串转时间对象;datetime.strftime()
...
class HotGoodsView(View):
    ...
    def get(self, request, category_id):
        # 查询指定分类的SKU信息, 而且必须是上架的状态,然后按照销量由高到低,而后切片取出前两位
...
class ListView(View):
    ...
    def get(self, request, category_id, page_num):
        ...
        try:
            # 三级类别
            category = GoodsCategory.objects.get(id=category_id)
        except GoodsCategory.objects.get(id=category_id):
        ...
        # 分页和排序查询:category查询sku,一查多,一方的模型类对象.多方关联字段.all/filter
